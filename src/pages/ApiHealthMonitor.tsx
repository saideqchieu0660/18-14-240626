...
export function ServiceMonitor({
  ...
  const [openRouterEnabled, setOpenRouterEnabled] = useState(
    () => localStorage.getItem("openRouterEnabled") !== "false",
  );
  const [groqEnabled, setGroqEnabled] = useState(
    () => localStorage.getItem("groqEnabled") !== "false",
  );
  const [geminiEnabled, setGeminiEnabled] = useState(
    () => localStorage.getItem("geminiEnabled") !== "false",
  );
  const [deepInfraEnabled, setDeepInfraEnabled] = useState(
    () => localStorage.getItem("deepInfraEnabled") !== "false",
  );
  ...
  const loadDeepInfraKeys = () => {
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      if (kp1 && typeof kp1 === "string" && kp1.trim())
        keysRaw.push(kp1.trim());

      const kp2 = safeProcessEnv[`VITE_DEEPINFRA_KEY_${i}`];
      if (kp2 && typeof kp2 === "string" && kp2.trim())
        keysRaw.push(kp2.trim());

      const kp3 = safeProcessEnv[`DEEPINFRA_API_KEY_${i}`];
      if (kp3 && typeof kp3 === "string" && kp3.trim())
        keysRaw.push(kp3.trim());
    }

    try {
      if (
        import.meta.env.VITE_DEEPINFRA_API_KEY &&
        typeof import.meta.env.VITE_DEEPINFRA_API_KEY === "string" &&
        import.meta.env.VITE_DEEPINFRA_API_KEY.trim()
      ) {
        keysRaw.push(import.meta.env.VITE_DEEPINFRA_API_KEY.trim());
      }
      if (
        import.meta.env.VITE_DEEPINFRA_KEY &&
        typeof import.meta.env.VITE_DEEPINFRA_KEY === "string" &&
        import.meta.env.VITE_DEEPINFRA_KEY.trim()
      ) {
        keysRaw.push(import.meta.env.VITE_DEEPINFRA_KEY.trim());
      }
    } catch (e) {}

    const uniqueKeys = Array.from(new Set(keysRaw))
      .map((k) => k.trim())
    ...
    try {
      ...
      if (res.ok) {
        ...
        localStorage.setItem(
          "openRouterEnabled",
          String(data.openRouterEnabled !== false),
        );
        localStorage.setItem("groqEnabled", String(data.groqEnabled !== false));
        localStorage.setItem(
          "geminiEnabled",
          String(data.geminiEnabled !== false),
        );
        localStorage.setItem(
          "deepInfraEnabled",
          String(data.deepInfraEnabled !== false),
        );
  ...
  const handleToggleChange = async (
    type: "openrouter" | "gemini" | "groq" | "deepinfra",
    newValue: boolean,
  ) => {
    const isSysAdmin =
    ...
    if (!isSysAdmin) {
      toast(
        "Ngài không phải Admin hệ thống, không có quyền bật tắt bộ ngắt mạch API toàn server đâu nhé!",
      );
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Direct Firebase save failed, falling back to backend only: ",
          fbErr,
        );
      }
      ...
      if (!res.ok) {
        if (!fbSuccess) {
          ...
          throw new Error(
            errorData.error ||
              "Không thể lưu trạng thái mới trên cả Frontend và Backend.",
          );
        }
        ...
        throw new Error(
          "Backend save failed, but Firebase fallback succeeded. Refreshing...",
        );
      }
      const data = await res.json();
      setOpenRouterEnabled(data.openRouterEnabled !== false);
  ...
  const clientLatencyAvg = useMemo(() => {
    const latencies = networkLogs
      .filter((log) => log.type === "latency")
  ...
  const fetchKeysStatus = async () => {
    ...
    try {
      ...
      if (!adminKey && !idToken) {
        setError(
          "Chưa nhập admin key hợp lệ hoặc không thể xác thực tài khoản để xem trạng thái API",
        );
        return;
      }
      ...
      setUsageHistory((prevHistory) => {
        ...
        data.keys.forEach((k: KeyState) => {
          const prevKey = prevKeysRef.current.find(
            (pk) => pk.index === k.index,
          );
        ...
        if (data.openrouter && data.openrouter.keys) {
          data.openrouter.keys.forEach((k: KeyState) => {
            ...
            const prevKey = prevOpenRouterKeysRef.current.find(
              (pk) => pk.index === k.index,
            );
            const currentUsage = k.usageCount;
            let delta = 0;
        ...
        if (data.groq && data.groq.keys) {
          data.groq.keys.forEach((k: KeyState) => {
            ...
            const prevKey = prevGroqKeysRef.current.find(
              (pk) => pk.index === k.index,
            );
            const currentUsage = k.usageCount;
            let delta = 0;
  ...
  const limitedCount = allKeys.filter(
  ...
  const statusScore =
    allKeys.length === 0
      ? 100
      : ((activeCount * 1 + limitedCount * 0.5 + failedCount * 0) /
          allKeys.length) *
        100;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="card-3d p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 flex flex-col items-center justify-center text-center">
          <div className="text-sm font-bold opacity-60 uppercase mb-2">
            API Connection Score
          </div>
            ...
            {allKeys.length > 0 && (
              <>
                <div
                  ...
                  style={{ width: `${(activeCount / allKeys.length) * 100}%` }}
                ></div>
                <div
                  className="h-full bg-orange-500 transition-all duration-500"
                  style={{ width: `${(limitedCount / allKeys.length) * 100}%` }}
                ></div>
                <div
                  className="h-full bg-red-500 transition-all duration-500"
                  style={{ width: `${(failedCount / allKeys.length) * 100}%` }}
                ></div>
              </>
            )}
          </div>
            ...
            <div className="bg-zinc-50 dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 flex flex-col justify-center">
              ...
              <div className="flex items-end gap-2 mb-1">
                <span className="text-3xl font-display font-bold text-blue-500">
                  {(
                    serverHealth.systemMemory.used /
                    1024 /
                    1024 /
                    1024
                  ).toFixed(1)}{" "}
                ...
                <span className="text-zinc-500 text-sm mb-1">
                  /{" "}
                  {(
                    serverHealth.systemMemory.total /
                    1024 /
                    1024 /
                    1024
                  ).toFixed(1)}{" "}
              ...
              <div className="w-full bg-zinc-200 dark:bg-zinc-800 rounded-full h-2 mt-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${(serverHealth.systemMemory.used / serverHealth.systemMemory.total) * 100}%`,
                  }}
                ></div>
              </div>
              <p className="text-xs text-zinc-500 mt-2">
                Node Process:{" "}
                {(serverHealth.processMemory.rss / 1024 / 1024).toFixed(1)} MB
              </p>
            </div>
            <div className="bg-zinc-50 dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 flex flex-col justify-center">
              <h3 className="text-sm font-bold opacity-60 uppercase mb-2 flex items-center gap-2">
              ...
              <div className="text-sm text-zinc-500">
                ...
                <span className="font-bold text-zinc-700 dark:text-zinc-300">
                  {serverHealth.cpus.length}
                </span>
              </div>
              <p className="text-xs text-zinc-500 mt-2">
                System Uptime: {Math.floor(serverHealth.uptime / 3600)}h{" "}
      ...
      <div className="flex flex-col justify-center bg-zinc-50 dark:bg-zinc-900 p-6 rounded-xl border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold flex items-center gap-2">
              <RefreshCw
                className={`w-5 h-5 text-blue-500 ${isPolling ? "animate-spin" : ""}`}
              />
            ...
            <p className="text-zinc-500 dark:text-zinc-400 text-sm mt-1">
              ...
              {totalKeys +
                totalOpenRouterKeys +
                totalGroqKeys +
                deepInfraKeys.length}{" "}
              API keys across{" "}
              {[
                totalKeys > 0 ? "Gemini" : "",
                totalOpenRouterKeys > 0 ? "OpenRouter" : "",
                totalGroqKeys > 0 ? "Groq" : "",
                deepInfraKeys.length > 0 ? "DeepInfra" : "",
              ]
                .filter(Boolean)
                .join(", ") || "all providers"}
              .
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${isResettingKeys ? "animate-spin" : ""}`}
              />
              {resetSuccess
        ...
        <div className="mt-4 p-4 border-t border-zinc-200 dark:border-zinc-800 flex items-center gap-4 text-sm font-mono">
          ...
          <div className="flex items-center gap-2">
            Processing:{" "}
            <span
              className={`font-bold ${queueStatus.isProcessing ? "text-green-500" : "text-zinc-500"}`}
            >
            ...
            <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-3">
              <div>
                ...
                <p className="text-xs text-zinc-500 mt-1 pb-1">
                  Chủ động ngắt cổng gọi ra ngoài của các nhà cung cấp nếu bị
                  block IP/quét hạn ngạch quá căng, tránh dính exceed request
                  thêm.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-1">
              ...
              >
                ...
                <button
                  ...
                  disabled={isUpdatingToggles}
                  onClick={() =>
                    handleToggleChange("openrouter", !openRouterEnabled)
                  }
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    openRouterEnabled
                      ? "bg-emerald-500"
              ...
              >
                ...
                <button
                  ...
                  disabled={isUpdatingToggles}
                  onClick={() =>
                    handleToggleChange("deepinfra", !deepInfraEnabled)
                  }
                  className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                    deepInfraEnabled
                      ? "bg-indigo-500"
          ...
          {isKeysEmpty && (
            <div className="flex flex-col items-center justify-center p-16 text-center space-y-4">
              ...
              <p className="text-zinc-500 max-w-md mx-auto tracking-wide font-light">
                Tất cả các hàng đợi API đều đang trống. Vui lòng cấu hình các
                khoá bí mật{" "}
                <code className="bg-zinc-100 dark:bg-zinc-800 px-1 py-0.5 rounded text-red-500 font-mono text-xs">
                  VITE_FIREBASE_API_KEY
                </code>
                , OpenRouter, Groq, hoặc DeepInfra để tái kích hoạt hệ thống
                truy vấn.
              </p>
              <button
                onClick={fetchKeysStatus}
                className="mt-4 px-6 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors flex items-center gap-2 font-medium tracking-wide"
              >
        ...
        <div className="card-3d rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden bg-white dark:bg-zinc-900/40">
          <div className="p-5 bg-zinc-50 border-b border-zinc-200 dark:bg-zinc-900/80 dark:border-zinc-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                Displays 429 status changes, cooling-down periods, and key
                health transitions
              </p>
            </div>
            ...
            <div className="flex bg-zinc-200/60 dark:bg-zinc-850 p-1 rounded-lg text-xs font-semibold">
              ...
              >
                Tất cả sự kiện (
                {logs.length + openRouterLogs.length + groqLogs.length})
              </button>
              <button
                type="button"
              ...
              >
                ...
                Lỗi 429 / Cooling-off (
                {
                  [...logs, ...openRouterLogs, ...groqLogs].filter((log) => {
          ...
          {(() => {
            ...
            const filteredLogs = combinedLogs.filter(
              (log) => logFilter === "all" || is429Event(log),
            );
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap items-center gap-2 shrink-0">
                        ...
                        <span
                          className={`font-mono border px-2.5 py-1 rounded-md text-xs font-bold ${
                            isOpenRouter
            ...
            <div className="card-3d p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-zinc-100 dark:border-zinc-800/60">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-bold ${serverHealth?.status === "UP" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-orange-100 text-orange-805 dark:bg-orange-900/30 dark:text-orange-400 animate-pulse"}`}
                  >
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="text-sm font-mono font-bold text-zinc-700 dark:text-zinc-300">
                    {serverHealth?.uptimeSeconds
                      ? `${Math.floor(serverHealth.uptimeSeconds / 60)}m ${serverHealth.uptimeSeconds % 60}s`
                      : "N/A"}
                  </span>
                </div>
              </div>
            ...
            <div className="card-3d p-6 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-zinc-100 dark:border-zinc-800/60">
                  ...
                  <span
                    className={`px-2 py-0.5 rounded text-xs font-bold ${navigator.onLine ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"}`}
                  >
            ...
            <div className="card-3d p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 flex flex-col h-full">
              ...
              {testResponse ? (
                <div className="flex-1">
                  <div className="text-xs text-zinc-500 mb-2 font-semibold uppercase">
                    Response JSON từ `/api/health`:
                  </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-zinc-400 dark:text-zinc-500">
                  ...
                  <p className="text-sm">
                    Bấm nút "Ping Test API" để thực hiện yêu cầu kiểm tra sức
                    khỏe (/api/health) và nhận payload JSON trực tiếp.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="card-3d p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/40 flex flex-col h-full">
              ...
              {networkLogs.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-zinc-400 dark:text-zinc-500">
                  ...
                  <p className="text-sm">
                    Chưa ghi nhận sự kiện kết nối nào. Trình duyệt tự đo độ trễ
                    origin định kỳ (60s).
                  </p>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          {log.type === "state_change" ? (
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] uppercase font-bold ${log.value === "online" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"}`}
                            >
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-4 text-center">
        ...
        <p className="text-zinc-500 max-w-sm mt-2">
          Strict Role-Based Access Gating enforced. You must be an administrator
